...
<nav aria-label="breadcrumb" style="padding-bottom:15px;">
    <ol class="breadcrumb">
      ...
      <li class="breadcrumb-item active" aria-current="page">Parent-Task-Details</li>
    </ol>
</nav>
            ...
            <div class="modal-body">
                ...
                <form >
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                          <input type="email" class="form-control" id="user_email" placeholder="User Email ID">
...
<script>
    let task_data;
    let mapping_id;
    let task_id_delete,assigne_id_delete,task_id_assign_task;
    ...
    function modalAssignTaskIDForAssignTask(id){
        task_id_assign_task = id;
        console.log(task_id_assign_task);
    }
    ...
    function getTask(taskID){
        let resp = $.ajax({
            type: 'GET',
            url: '/task_with_subtask_with_user/'+taskID
        });
        resp.done(function(resp){
            // console.log(resp.task.title);
            // console.log(resp.task.sub_task);
            // for(let i=0;i<resp.task.sub_task.length;i++){
            //     console.log(resp.task.sub_task[i]);
            // }
            // return;
            html = `<h3 class="text-center" style="padding-top:20px;">${resp.task.title}</h3>`;
            for(let i=0;i<resp.task.sub_task.length;i++){
                $task = resp.task.sub_task[i];
                html += `
                    <div class="card-separator" style="padding-top:40px;"></div>
                    <div class='card'>
                        <div class="card-header">
                            <div class = "row">
                                ...
                                <div class="col-12 col-sm-4">
                                    <button type="button" class="btn btn-secondary btn-sm" data-toggle="modal" data-target="#createAssignModal" onclick="modalAssignTaskIDForAssignTask('${$task.id}')">
                                        Assign <i class="fa fa-plus" style="margin-left: 5px;"></i>
                                    </button>
    ...
    function getUserDetails(user_id){
        ...
        resp.done(function(resp){
            ...
            html += '<div class="user-tasks-container">';
